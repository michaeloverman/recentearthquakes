...
public class ParseFeed {
	...
	public static List<PointFeature> parseEarthquake(PApplet p, String fileName) {
		...
		for (int i = 0; i < itemXML.length; i++) {
				...
				// find time of eq
				point.putProperty("time", findTime(itemXML[i]));
	...
	private static Calendar findTime(XML itemXML) {
		// date/time pattern to find in the data
		final String REGEX = "\\d\\d\\d\\d-\\d\\d-\\d\\d \\d\\d:\\d\\d:\\d\\d";
		
		String item = itemXML.toString();
		Pattern p = Pattern.compile(REGEX);
		Matcher m = p.matcher(item);
		
//		String timeString = findTimeString(itemXML[i]);
//		DateFormat df = new SimpleDateFormat("yyyy.MM.dd hh:mm:ss");
		if (m.find()) {
			String[] apart = item.substring(m.start(), m.end()).split("[:\\s-]");
//		System.out.println(apart[0] + " " + apart[1] + " " + apart[2] + " " + apart[3] + " " + apart[4]);
			Calendar date = new Calendar.Builder()
					.setDate(Integer.parseInt(apart[0]),
					Integer.parseInt(apart[1]) - 1,
					Integer.parseInt(apart[2]))
					.setTimeOfDay(Integer.parseInt(apart[3]),
					Integer.parseInt(apart[4]),
					Integer.parseInt(apart[5]))
					.build();
			return date;
		} else return null;
