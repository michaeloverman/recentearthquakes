		...
		implements Comparable<EarthquakeMarker> 
{
	...
	public static final float FLAG_SIZE = 14;
	...
	public void drawMarker(PGraphics pg, float x, float y) {
		...
		if (flagOn && ranked) drawFlag(pg, x, y);
	...
	/** Show a flag denoting rank if among the strongest quakes shown */
	public void drawFlag(PGraphics pg, float x, float y) {
		String number = Integer.toString(getRank());
		float width = pg.textWidth(number) + 6;
		float height = pg.textAscent() + pg.textDescent();
		
		// pole
		pg.fill(0,0,0);
		pg.strokeWeight(4);
		pg.line(x, y-radius, x, y-radius-15-height);
		
		// flag
		pg.fill(200,50,0);
		pg.stroke(200,50,0);
		pg.strokeWeight(1);
		pg.rect(x+1, y-radius-15-height, width, height);
		
		// number
		pg.fill(0,0,0);
		pg.text(number, x+3, y-radius-18);
